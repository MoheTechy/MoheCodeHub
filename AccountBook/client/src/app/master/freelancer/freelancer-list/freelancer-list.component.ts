...
export class FreelancerListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.FreelancerService.getRecord().then(results => {
      if (!results) {
        return
      }
      this.temp = results;
  ...
  btnAddNewClick = () => {
    let dialogRef = this.dialog.open(FreelancerFormComponent, {
      data: {},
      disableClose: false
    });
  ...
  deleteTable = (item) => {
    ...
    bottomSheetRef.afterDismissed().subscribe(result => {
      if (result === true) {
        ...
        this.FreelancerService.deleteRecord(item).subscribe(res => {
          if (res['statusBool'] == 1) {
